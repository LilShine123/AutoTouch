import tkinter as tk
from tkinter import messagebox, filedialog
import pyautogui
import threading
import time
import math
import keyboard
import json
import os

class MultiPointClickerGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("自动连点器")
        self.root.geometry("500x450")
        self.root.minsize(500, 450)

        self.points = []          # [x, y, repeat]
        self.markers = []
        self.is_running = False
        self.click_thread = None
        self.stop_tracking = False

        self.mouse_move_threshold = 300
        self.last_mouse_pos = None

        self.timer_thread = None
        self.timer_running = False

        self.total_rounds = 0
        self.stop_reason = ""

        self.control_buttons = []
        self.modify_buttons = []

        self.setup_ui()
        self.update_mouse_position()

        try:
            keyboard.add_hotkey('space', self.add_point)
            keyboard.add_hotkey('esc', self.toggle_click)
        except Exception as e:
            messagebox.showwarning("警告", f"全局热键注册失败，请以管理员权限运行。\n错误: {e}")

        self.root.protocol("WM_DELETE_WINDOW", self.on_closing)

    def setup_ui(self):
        # 第一行：控制按钮 + 复选框
        row1 = tk.Frame(self.root)
        row1.pack(pady=6)

        btn_add = tk.Button(row1, text="添加 (空格)", command=self.add_point,
                            width=12, bg="#4CAF50", fg="white", font=("Arial", 10, "bold"))
        btn_add.pack(side=tk.LEFT, padx=2)
        self.control_buttons.append(btn_add)

        btn_del = tk.Button(row1, text="删除选中", command=self.delete_selected,
                            width=12, bg="#f44336", fg="white", font=("Arial", 10, "bold"))
        btn_del.pack(side=tk.LEFT, padx=2)
        self.control_buttons.append(btn_del)

        btn_clear = tk.Button(row1, text="清空", command=self.clear_points,
                              width=10, bg="#ff9800", fg="white", font=("Arial", 10, "bold"))
        btn_clear.pack(side=tk.LEFT, padx=2)
        self.control_buttons.append(btn_clear)

        self.topmost_var = tk.IntVar(value=1)
        self.loop_var = tk.IntVar(value=1)

        chk_top = tk.Checkbutton(row1, text="置顶", variable=self.topmost_var,
                                 command=self.toggle_topmost, font=("Arial", 9))
        chk_top.pack(side=tk.LEFT, padx=(8, 0))

        chk_loop = tk.Checkbutton(row1, text="循环", variable=self.loop_var,
                                  font=("Arial", 9))
        chk_loop.pack(side=tk.LEFT, padx=8)

        self.root.attributes('-topmost', True)

        # 第二行：参数设置
        row2 = tk.Frame(self.root)
        row2.pack(pady=4)

        tk.Label(row2, text="每秒次数:", font=("Arial", 9)).pack(side=tk.LEFT, padx=2)
        self.rate_var = tk.StringVar(value="2.0")
        tk.Entry(row2, textvariable=self.rate_var, width=6).pack(side=tk.LEFT, padx=2)
        tk.Label(row2, text="次/秒", font=("Arial", 9)).pack(side=tk.LEFT, padx=2)

        tk.Label(row2, text="点间间隔(秒):", font=("Arial", 9)).pack(side=tk.LEFT, padx=(10,2))
        self.between_interval_var = tk.StringVar(value="0.5")
        tk.Entry(row2, textvariable=self.between_interval_var, width=6).pack(side=tk.LEFT, padx=2)

        tk.Label(row2, text="定时(分钟):", font=("Arial", 9)).pack(side=tk.LEFT, padx=(10,2))
        self.timer_var = tk.StringVar(value="0")
        tk.Entry(row2, textvariable=self.timer_var, width=5).pack(side=tk.LEFT, padx=2)
        self.timer_btn = tk.Button(row2, text="定时", command=self.start_timer,
                                   width=6, bg="#FFA500", fg="white", font=("Arial", 9, "bold"))
        self.timer_btn.pack(side=tk.LEFT, padx=2)

        # 第三行：点数、鼠标位置、修改次数
        row3 = tk.Frame(self.root)
        row3.pack(pady=4)

        self.count_label = tk.Label(row3, text="当前点数: 0", font=("Arial", 9))
        self.count_label.pack(side=tk.LEFT, padx=10)

        self.pos_label = tk.Label(row3, text="鼠标位置: (0, 0)", font=("Arial", 9))
        self.pos_label.pack(side=tk.LEFT, padx=15)

        tk.Label(row3, text="修改次数:", font=("Arial", 9)).pack(side=tk.LEFT, padx=(15,2))
        self.repeat_var = tk.StringVar(value="1")
        tk.Entry(row3, textvariable=self.repeat_var, width=5).pack(side=tk.LEFT, padx=2)
        btn_apply = tk.Button(row3, text="应用", command=self.apply_repeat,
                              width=6, bg="#9C27B0", fg="white", font=("Arial", 9, "bold"))
        btn_apply.pack(side=tk.LEFT, padx=2)
        self.modify_buttons.append(btn_apply)

        # 状态栏
        self.status_label = tk.Label(self.root, text="状态: 停止", fg="gray", font=("Arial", 9))
        self.status_label.pack(pady=2)

        # 点列表
        list_frame = tk.Frame(self.root)
        list_frame.pack(pady=5, fill=tk.BOTH, expand=True, padx=20)
        scrollbar = tk.Scrollbar(list_frame)
        scrollbar.pack(side=tk.RIGHT, fill=tk.Y)
        self.listbox = tk.Listbox(list_frame, yscrollcommand=scrollbar.set, height=10,
                                  font=("Arial", 9))
        self.listbox.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)
        scrollbar.config(command=self.listbox.yview)

        # 底行：开始、操作说明、存档按钮
        row5 = tk.Frame(self.root)
        row5.pack(pady=6)

        self.toggle_btn = tk.Button(row5, text="▶ 开始", command=self.toggle_click,
                                    width=10, bg="#2196F3", fg="white", font=("Arial", 10, "bold"))
        self.toggle_btn.pack(side=tk.LEFT, padx=3)

        help_btn = tk.Button(row5, text="📖 操作说明", command=self.show_help,
                             width=12, bg="#607D8B", fg="white", font=("Arial", 10, "bold"))
        help_btn.pack(side=tk.LEFT, padx=3)

        save_btn = tk.Button(row5, text="💾 保存配置", command=self.save_config,
                             width=12, bg="#388E3C", fg="white", font=("Arial", 10, "bold"))
        save_btn.pack(side=tk.LEFT, padx=3)

        load_btn = tk.Button(row5, text="📂 加载配置", command=self.load_config,
                             width=12, bg="#1976D2", fg="white", font=("Arial", 10, "bold"))
        load_btn.pack(side=tk.LEFT, padx=3)

        tk.Label(self.root, text="全局热键: 空格添加 | ESC切换 | 晃动鼠标自动停止 | 配置保存为 .json",
                 font=("Arial", 8), fg="blue").pack(pady=(0, 5))
        tk.Label(self.root, text="提示: 标记可拖动（鼠标左键拖拽）自动更新坐标",
                 font=("Arial", 8), fg="gray").pack()

    # ---------- 存档功能 ----------
    def save_config(self):
        if self.is_running:
            messagebox.showwarning("警告", "点击运行中不允许保存配置")
            return
        if not self.points:
            messagebox.showwarning("警告", "当前没有点可保存")
            return
        file_path = filedialog.asksaveasfilename(
            defaultextension=".json",
            filetypes=[("JSON files", "*.json"), ("All files", "*.*")],
            title="保存配置"
        )
        if not file_path:
            return
        try:
            data = {"points": self.points}
            with open(file_path, 'w', encoding='utf-8') as f:
                json.dump(data, f, indent=2, ensure_ascii=False)
            messagebox.showinfo("成功", f"配置已保存到：{file_path}")
        except Exception as e:
            messagebox.showerror("错误", f"保存失败：{str(e)}")

    def load_config(self):
        if self.is_running:
            messagebox.showwarning("警告", "点击运行中不允许加载配置")
            return
        file_path = filedialog.askopenfilename(
            defaultextension=".json",
            filetypes=[("JSON files", "*.json"), ("All files", "*.*")],
            title="加载配置"
        )
        if not file_path:
            return
        try:
            with open(file_path, 'r', encoding='utf-8') as f:
                data = json.load(f)
            new_points = data.get("points", [])
            if not isinstance(new_points, list) or not all(isinstance(p, list) and len(p) == 3 for p in new_points):
                raise ValueError("无效的配置格式")
            # 清空当前
            for marker in self.markers:
                try:
                    marker.destroy()
                except:
                    pass
            self.markers.clear()
            self.points.clear()
            for x, y, repeat in new_points:
                self.points.append([x, y, repeat])
                marker = self.create_marker(x, y, len(self.points))
                self.markers.append(marker)
            self.update_listbox()
            self.count_label.config(text=f"当前点数: {len(self.points)}")
            messagebox.showinfo("成功", f"成功加载 {len(self.points)} 个点")
        except Exception as e:
            messagebox.showerror("错误", f"加载失败：{str(e)}")

    # ---------- 控制按钮启用/禁用 ----------
    def set_controls_enabled(self, enabled):
        state = tk.NORMAL if enabled else tk.DISABLED
        for btn in self.control_buttons:
            try:
                btn.config(state=state)
            except:
                pass
        for btn in self.modify_buttons:
            try:
                btn.config(state=state)
            except:
                pass

    # ---------- 置顶 ----------
    def toggle_topmost(self):
        if self.topmost_var.get() == 1:
            self.root.attributes('-topmost', True)
        else:
            self.root.attributes('-topmost', False)
        for marker in self.markers:
            try:
                marker.attributes('-topmost', self.root.attributes('-topmost'))
            except:
                pass

    # ---------- 修改次数 ----------
    def apply_repeat(self):
        if self.is_running:
            messagebox.showwarning("警告", "点击运行中不允许修改次数")
            return
        selection = self.listbox.curselection()
        if not selection:
            messagebox.showwarning("警告", "请先在列表中选中一个点")
            return
        try:
            new_repeat = int(self.repeat_var.get())
            if new_repeat < 1:
                raise ValueError
        except:
            messagebox.showwarning("警告", "请输入有效的正整数（≥1）")
            return
        index = selection[0]
        if 0 <= index < len(self.points):
            self.points[index][2] = new_repeat
            self.update_listbox()

    # ---------- 定时 ----------
    def start_timer(self):
        try:
            minutes = float(self.timer_var.get())
        except:
            messagebox.showwarning("警告", "请输入有效的分钟数（数字）")
            return
        if minutes <= 0:
            messagebox.showwarning("警告", "分钟数必须大于0")
            return
        if self.timer_running:
            messagebox.showinfo("提示", "定时已在运行中")
            return
        if not self.is_running:
            self.start_click()
            if not self.is_running:
                return
        self.timer_running = True
        self.timer_btn.config(text="定时中...", state=tk.DISABLED)
        self.status_label.config(text=f"定时 {minutes} 分钟后停止", fg="purple")

        def timer_loop():
            time.sleep(minutes * 60)
            if self.timer_running:
                self.stop_reason = "定时已到"
                self.root.after(0, self.stop_click)
        self.timer_thread = threading.Thread(target=timer_loop, daemon=True)
        self.timer_thread.start()

    # ---------- 标记（可拖拽，无偏移） ----------
    def create_marker(self, x, y, index):
        marker = tk.Toplevel(self.root)
        marker.overrideredirect(True)
        marker.attributes('-topmost', self.root.attributes('-topmost'))
        marker.attributes('-alpha', 0.75)
        marker.config(bg='yellow')
        label = tk.Label(marker, text=str(index), font=("Arial", 12, "bold"),
                         bg='yellow', fg='black', padx=4, pady=2)
        label.pack()
        marker.index = index - 1
        marker.offset_x = 0
        marker.offset_y = 0

        label.bind('<Button-1>', lambda e, m=marker: self.on_marker_press(e, m))
        label.bind('<B1-Motion>', lambda e, m=marker: self.on_marker_drag(e, m))
        label.bind('<ButtonRelease-1>', lambda e, m=marker: self.on_marker_release(e, m))
        marker.bind('<Button-1>', lambda e, m=marker: self.on_marker_press(e, m))
        marker.bind('<B1-Motion>', lambda e, m=marker: self.on_marker_drag(e, m))
        marker.bind('<ButtonRelease-1>', lambda e, m=marker: self.on_marker_release(e, m))

        # ---------- 精确对齐，无偏移 ----------
        marker.geometry(f"+{x}+{y}")

        marker.current_x = x
        marker.current_y = y
        return marker

    def on_marker_press(self, event, marker):
        marker.offset_x = event.x
        marker.offset_y = event.y

    def on_marker_drag(self, event, marker):
        x = event.x_root - marker.offset_x
        y = event.y_root - marker.offset_y
        marker.geometry(f"+{x}+{y}")
        click_x = x
        click_y = y
        marker.current_x = click_x
        marker.current_y = click_y

    def on_marker_release(self, event, marker):
        idx = marker.index
        if 0 <= idx < len(self.points):
            self.points[idx][0] = marker.current_x
            self.points[idx][1] = marker.current_y
            self.update_listbox()
            self.count_label.config(text=f"当前点数: {len(self.points)}")

    # ---------- 点管理 ----------
    def add_point(self):
        if self.is_running:
            messagebox.showwarning("警告", "点击运行中不允许添加点")
            return
        x, y = pyautogui.position()
        self.points.append([x, y, 1])
        marker = self.create_marker(x, y, len(self.points))
        self.markers.append(marker)
        self.update_listbox()
        self.listbox.see(tk.END)
        self.count_label.config(text=f"当前点数: {len(self.points)}")

    def delete_selected(self):
        if self.is_running:
            messagebox.showwarning("警告", "点击运行中不允许删除点")
            return
        selection = self.listbox.curselection()
        if selection:
            index = selection[0]
            if index < len(self.markers):
                self.markers[index].destroy()
                del self.markers[index]
            del self.points[index]
            self.rebuild_markers()
            self.update_listbox()
            self.count_label.config(text=f"当前点数: {len(self.points)}")

    def clear_points(self):
        if self.is_running:
            messagebox.showwarning("警告", "点击运行中不允许清空点")
            return
        if self.points:
            if messagebox.askyesno("确认", "确定要清空所有点击点吗？"):
                for marker in self.markers:
                    try:
                        marker.destroy()
                    except:
                        pass
                self.markers.clear()
                self.points.clear()
                self.listbox.delete(0, tk.END)
                self.count_label.config(text="当前点数: 0")

    def rebuild_markers(self):
        for marker in self.markers:
            try:
                marker.destroy()
            except:
                pass
        self.markers.clear()
        for i, (x, y, _) in enumerate(self.points, 1):
            marker = self.create_marker(x, y, i)
            marker.index = i - 1
            self.markers.append(marker)

    def update_listbox(self):
        self.listbox.delete(0, tk.END)
        for i, (x, y, repeat) in enumerate(self.points, 1):
            self.listbox.insert(tk.END, f"点 {i}: ({x}, {y}) 重复 {repeat} 次")

    # ---------- 操作说明 ----------
    def show_help(self):
        help_text = """
        📌 操作说明

        1. 添加点击点
           • 将鼠标移到目标位置，按【空格键】（全局热键）或点击“添加 (空格)”按钮
           • 添加后会在该位置生成一个半透明数字标记，标记左上角精确对齐点击坐标
           • 支持鼠标拖拽标记调整位置（左键拖拽）

        2. 开始 / 停止点击
           • 点击“▶ 开始”按钮 或 按【ESC键】（全局）开始循环点击
           • 点击“⏹ 停止”按钮（按钮会切换）或 再次按【ESC键】停止
           • 停止时会显示总共执行的轮数及原因

        3. 窗口置顶
           • 勾选“置顶”复选框使窗口始终在最前面（默认开启）

        4. 自动停止功能
           • 点击过程中若手动移动鼠标超过 300 像素，程序自动停止
           • 将鼠标快速移到屏幕左上角也可紧急停止

        5. 管理点击点
           • 选中列表中的点，点击“删除选中”可移除（标记同步消失）
           • 点击“清空”清除全部点（标记同步消失）

        6. 速度与间隔
           • “每秒次数”控制单个点内部多次点击的频率（次/秒）
           • “点间间隔”控制上一个点执行完毕后到下一个点开始的等待时间（秒）

        7. 循环控制
           • 勾选“循环”时，一轮结束后自动回到第一个点继续
           • 取消勾选时，执行完一轮后自动停止

        8. 定时停止
           • 在“定时(分钟)”中输入时长，点击“定时”按钮开始倒计时
           • 时间到后自动停止点击（若未开始则先自动开始）

        9. 存档功能
           • “保存配置”将当前所有点击点保存为 .json 文件
           • “加载配置”从 .json 文件恢复点击点
        """
        messagebox.showinfo("操作说明", help_text)

    # ---------- 核心控制 ----------
    def toggle_click(self):
        if self.is_running:
            self.stop_reason = "手动停止"
            self.stop_click()
        else:
            self.start_click()

    def start_click(self):
        if self.is_running:
            return
        if not self.points:
            messagebox.showwarning("警告", "请先添加点击点！")
            return
        self.is_running = True
        self.total_rounds = 0
        self.stop_reason = ""
        self.toggle_btn.config(text="⏹ 停止", bg="#f44336", fg="white")
        self.status_label.config(text="状态: 启动中...", fg="green")
        try:
            self.last_mouse_pos = pyautogui.position()
        except:
            self.last_mouse_pos = None

        self.set_controls_enabled(False)

        self.click_thread = threading.Thread(target=self.click_loop, daemon=True)
        self.click_thread.start()

    def stop_click(self):
        if not self.is_running:
            return
        self.is_running = False
        self.toggle_btn.config(text="▶ 开始", bg="#2196F3", fg="white")
        self.set_controls_enabled(True)

        reason = self.stop_reason if self.stop_reason else "手动停止"
        total = self.total_rounds
        self.status_label.config(text=f"已停止：{reason}，共执行 {total} 轮", fg="gray")
        self.last_mouse_pos = None
        if self.timer_running:
            self.timer_running = False
            self.timer_btn.config(text="定时", state=tk.NORMAL)

    # ---------- 点击循环 ----------
    def click_loop(self):
        try:
            rate = float(self.rate_var.get())
            if rate <= 0:
                rate = 2.0
        except:
            rate = 2.0
        interval = 1.0 / rate

        try:
            between = float(self.between_interval_var.get())
            if between < 0:
                between = 0.0
        except:
            between = 0.0

        loop = (self.loop_var.get() == 1)

        self.update_status("开始执行点击...", "green")

        click_count = 0
        round_count = 0
        while self.is_running:
            round_count += 1
            self.total_rounds = round_count
            points_snapshot = self.points.copy()
            if not points_snapshot:
                self.stop_reason = "点列表为空"
                self.root.after(0, self.stop_click)
                break

            for i, (x, y, repeat) in enumerate(points_snapshot, 1):
                if not self.is_running:
                    break
                for _ in range(repeat):
                    if not self.is_running:
                        break
                    try:
                        pyautogui.click(x, y)
                        click_count += 1
                        self.update_status(
                            f"第{round_count}轮 点{i}/{len(points_snapshot)} (共{click_count}次)",
                            "green"
                        )
                        try:
                            self.last_mouse_pos = pyautogui.position()
                        except:
                            pass
                        time.sleep(interval)
                    except pyautogui.FailSafeException:
                        self.stop_reason = "鼠标移到左上角（紧急停止）"
                        self.root.after(0, self.stop_click)
                        return
                    except Exception as e:
                        self.stop_reason = f"点击异常（坐标{x},{y}）: {str(e)}"
                        self.root.after(0, self.stop_click)
                        return
                if i < len(points_snapshot):
                    time.sleep(between)
                    try:
                        self.last_mouse_pos = pyautogui.position()
                    except:
                        pass
                if self.last_mouse_pos is not None:
                    try:
                        cur_x, cur_y = pyautogui.position()
                        prev_x, prev_y = self.last_mouse_pos
                        distance = math.hypot(cur_x - prev_x, cur_y - prev_y)
                        if distance >= self.mouse_move_threshold:
                            self.stop_reason = f"检测到鼠标晃动（位移{distance:.0f}像素），自动停止"
                            self.root.after(0, self.stop_click)
                            return
                    except:
                        pass
            if not loop:
                self.stop_reason = f"一轮完成（第{round_count}轮），未勾选循环"
                self.root.after(0, self.stop_click)
                break
        if self.is_running:
            self.root.after(0, self.stop_click)

    def update_status(self, text, color=None):
        self.root.after(0, lambda: self.status_label.config(text=text, fg=color if color else "black"))

    def update_mouse_position(self):
        if not self.stop_tracking:
            try:
                x, y = pyautogui.position()
                self.pos_label.config(text=f"鼠标位置: ({x}, {y})")
            except:
                pass
            self.root.after(100, self.update_mouse_position)

    def on_closing(self):
        self.stop_tracking = True
        self.is_running = False
        self.timer_running = False
        try:
            keyboard.remove_all_hotkeys()
        except:
            pass
        for marker in self.markers:
            try:
                marker.destroy()
            except:
                pass
        self.markers.clear()
        self.root.destroy()

if __name__ == "__main__":
    root = tk.Tk()
    app = MultiPointClickerGUI(root)
    root.mainloop()
